...
- `<skill-dir>/references/memory-retrieval.md` — memory/retrieval engines: fact lifecycle and supersession, ingestion and chunk provenance, embedding lifecycle, retrieval pipeline and defaults, relation graphs, tenancy scoping, retrieval quality evaluation.
...
6. Documentation critic
   ...
   - Every critic verdict must open with a scope statement — what was examined, by what method, and what was NOT examined — and must never assert a complete residue ("the only defects are…", "the findings above are the complete residue"). A verdict can only vouch for what its pass actually inspected; completeness claims convert unexamined gaps into affirmative coverage claims, which is worse than silence.
   - Record the critic's verdicts verbatim in the `Structure-fit review` and `Self-containment review` sections of `appendix-coverage-and-evidence.md`. The quoted verdict is immutable provenance — never edit it after the fact. Below it, maintain a separate disposition table (finding → fixed/dismissed/waived, with file:line) that describes the CURRENT text; after any post-verdict fix, update that table and re-run the validator, so the verdict stays honest history and the table stays honest present. The validator requires both sections. When no subagents are available, perform this pass yourself after a break from drafting and record it the same way.
...
- A `Self-containment review` section: the critic's verbatim verdict on reading-order quality — forward dependencies, thin definitions, wrong-mental-model traps, two-sense terms — plus a current disposition table for every validator `self-containment:` warning and any waivers (see "Self-Containment"). The verdict is immutable; the disposition table must describe the shipped text and be refreshed after any post-verdict fix. The validator requires this section and rejects bare waivers.
